...
if not os.path.exists("./data/IMDB/aclImdb"):
    ...
    result = tfile.extractall("./data/IMDB/")
...
"""
# # 建立模型人工神经网络ANN模型（也称多层感知机 MPLP）"""

model = tf.keras.Sequential()

# 这一步很重要，文字转换为数字后，数字在语义上无任何关联。
# 我们需要将每一个“数字”映射到一个向量空间中去，让它们语义相近的
# 聚在一起，比较接近。
# 下面我们将每一个数字映射到32维的向量空间中去：output_dim=32

# 嵌入层
model.add(tf.keras.layers.Embedding(output_dim=32, input_dim=2000, input_length=100))

# 输入层（平坦）
model.add(tf.keras.layers.Flatten())
model.add(tf.keras.layers.Dropout(0.2))  # 剪枝25%
...
t2 = time.time()
ANNIMDB = float(t2 - t1)
print("Time taken: {} seconds".format(ANNIMDB))
...
predict_result = np.argmax(model.predict(pad_input_seq), axis=1)
predict_result[0]
SentimentDict[predict_result[0]]
...
with open("./out/IMDB/tf_IMDB100_ANN_model.json", "w") as json_file:
    json_file.write(model_json)

model.save_weights("./out/IMDB/tf_IMDB100_ANN_model.h5")
