...
class Mnist:
    ...
    label_names = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]  # 标签的字符串信息
...
if __name__ == "__main__":
    root = Path("F:\DL-Demo\Computer-Vision\Dataset\Multi-Classification")  # 数据集根目录

    mnist = Mnist(root=root / "MNIST/MNIST", vis_sample=True)
    mnist.get_categories(root / "MNIST/MNIST")
    # mnist.save_data(root + "MNIST")

    fashion_mnist = FashionMnist(root=root / "MNIST/FashionMNIST", vis_sample=True)
    fashion_mnist.get_categories(root / "MNIST/FashionMNIST")
    # fashion_mnist.save_data(root + "FashionMNIST")

    cifar10 = Cifar10(root=root / "CIFAR/CIFAR10", vis_sample=True)
    cifar10.get_categories(root / "CIFAR/CIFAR10")
    # cifar10.save_data(root + "CIFAR10")

    cifar100 = Cifar100(root=root / "CIFAR/CIFAR100", vis_sample=True)
    cifar100.get_categories(root / "CIFAR/CIFAR100")
