...
""""@@@ 网络块 DenseNet201 """
# 建立卷积神经网络模型（DenseNet201）
# 注意：原输入224X224，现调整为：32X32。 输出为1000，调整为：10
num_layers = [6, 12, 48, 32]  # 卷积层个数
...
# 输出模型列表，需要调整近1000万的参数，Total params: 9,760,650
print(model.summary())

""""@@@ 训练模型 """
epochs = 1  # 时间：2分钟  训练集准确率：28.83%   测试集准确率：13.01%
...
t2 = time.time()
CNNDenseNet201 = float(t2 - t1)
print("Time taken: {} seconds".format(CNNDenseNet201))
...
open('./out/Cifar10/DenseNet201_architecture.json', 'w').write(model_json)
model.save_weights('./out/Cifar10/DenseNet201_weights.h5', overwrite=True)
