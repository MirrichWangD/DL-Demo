...
""""@@@ 网络块 DenseNet264 """
# 建立卷积神经网络模型（DenseNet264）
# 注意：原输入224X224，现调整为：32X32。 输出为1000，调整为：10
num_layers = [6, 12, 64, 48]  # 卷积层个数
...
x = tf.keras.layers.AveragePooling2D(pool_size=(1, 1), strides=1)(x)
# Block4
for i in range(num_layers[3]):
    if i == 0:
        x0 = x
...
# 输出模型列表，需要调整1300万的参数，Total params: 13,657,482
print(model.summary())

""""@@@ 训练模型 """
epochs = 1  # 时间：2分钟  训练集准确率：28.48%   测试集准确率：13.59%
...
t2 = time.time()
CNNDenseNet264 = float(t2 - t1)
print("Time taken: {} seconds".format(CNNDenseNet264))
...
open("./out/Cifar10/DenseNet264_architecture.json", "w").write(model_json)
model.save_weights("./out/Cifar10/DenseNet264_weights.h5", overwrite=True)
