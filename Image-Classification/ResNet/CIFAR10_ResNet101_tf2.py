...
""""@@@ 网络块 ResNet101 """
# 建立卷积神经网络模型（ResNet101）
# 注意：原输入224X224，现调整为：32X32。 输出为1000，调整为：10
num_layers = [3, 4, 23, 3] #卷积层个数
...
for i in range(num_layers[2]):
    ...
    x = tf.keras.layers.ReLU()(x)
    if i == 0:
        x = tf.keras.layers.Conv2D(filters=256, kernel_size=(3, 3), strides=2,
                                   padding='same')(x)
    else:
...
for i in range(num_layers[3]):
    ...
    x = tf.keras.layers.ReLU()(x)
    if i == 0:
        x = tf.keras.layers.Conv2D(filters=512, kernel_size=(3, 3), strides=2,
                                   padding='same')(x)
    else:
...
#输出模型列表，需要调整4000万的参数，Total params: 42,678,666
print(model.summary())



""""@@@ 训练模型 """
epochs=1   #时间：1分钟  训练集准确率：36.81%   测试集准确率：9.52%
...
t2=time.time()
CNNResNet101 = float(t2-t1)
print("Time taken: {} seconds".format(CNNResNet101))
...
open('18DL/SaveModel/ResNet101_architecture.json', 'w').write(model_json)
model.save_weights('18DL/SaveModel/ResNet101_weights.h5', overwrite=True)
