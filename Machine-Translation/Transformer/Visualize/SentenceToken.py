...
dataset = TranslationDataset(file_path="../Tatoeba-Translation/data/cmn-eng.txt", src_lang="en", tgt_lang="zh")
print(dataset)

with open("../Tatoeba-Translation/data/eng_dict.txt", "w", encoding="utf-8") as f:
    f.writelines(map(lambda i: i + "\n", dataset.src_vocab.get_itos()))

with open("../Tatoeba-Translation/data/cmn_dict.txt", "w", encoding="utf-8") as f:
    f.writelines(map(lambda i: i + "\n", dataset.src_vocab.get_itos()))

# 划分训练、验证、测试集，分批
indices = np.arange(len(dataset))[::-1]
batch_sampler = BatchSampler(indices, bs, False)
...
for idx in range(0, 4):
    ...
    ax[idx].set_yticks(np.linspace(0.5, seq_len - 0.5, seq_len), rotation=0, labels=src_sentence)
    ax1[idx].set_yticks(np.linspace(0.5, seq_len - 0.5, seq_len), rotation=0, labels=tgt_sentence)
    # ax[idx+1].vlines(1, 0, 100)
    # plt.vlines(5, 0, 10)

# fig.text(0.5, .01, "句子索引", ha="center")
# fig.text(.01, 0.5, "词索引", va="center", rotation=90)
fig.suptitle("英文Token向量")
fig.tight_layout()
fig.savefig("imgs/eng-Token.png")

fig1.suptitle("繁体中文Token向量")
fig1.tight_layout()
fig1.savefig("imgs/cmn-Token.png")
